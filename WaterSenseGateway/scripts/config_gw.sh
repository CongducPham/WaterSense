...
#
# Adapted for the WaterSense project by Nestle on April 2017.
#
...
if [ "$ouinon" = "y" ] || [ "$ouinon" = "Y" ]
	then
		...
		sudo sed -i 's/^ssid.*/ssid=WS_PI_GW_'"$1"'/g' /etc/hostapd/hostapd.conf
		echo "Done"
		echo "Gateway WiFi ssid is WS_PI_GW_$1"
...
if [ "$ouinon" = "y" ] || [ "$ouinon" = "Y" ]
	then
		...
		sudo sed -i 's/^#Name *=.*/Name=WS_PI_BT_GW_'"$1"'/g' /etc/bluetooth/main.conf
		# then in all cases we replace Name so that if we already have an uncommented Name it will also work
		sudo sed -i 's/^Name *=.*/Name=WS_PI_BT_GW_'"$1"'/g' /etc/bluetooth/main.conf
		echo "Done"
		echo "Gateway Bluetooth network name is WS_PI_BT_GW_$1"
fi
